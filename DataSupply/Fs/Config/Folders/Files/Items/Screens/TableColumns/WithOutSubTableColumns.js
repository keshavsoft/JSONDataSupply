        ...
        ([key, value]) => {
            ...
            if ("TableColumns" in value) {
                if ("SubTableColumns" in value) {
                    LocalLoopObject.TableColumnsObject = PrepareTableColumnObjectWithOutSubtTableColumns({
                        inData: value.TableColumns,
                        inSubTableColumns: value.SubTableColumns
                    });
                } else {
...
let PrepareTableColumnObjectWithOutSubtTableColumns = ({ inData, inSubTableColumns }) => {
    let LocalReturnObject = {};
    let LocalSubTableColumnsArray = Object.keys(inSubTableColumns);
    let LocalShowColumns = inData.filter(element => LocalSubTableColumnsArray.includes(element.DataAttribute) === false);
    console.log("LocalShowColumns : ", LocalShowColumns.length);

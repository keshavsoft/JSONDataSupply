...
let RowsAsObjectsWithColumns = ({ inFolderName, inFileNameWithExtension, inItemName, inUserPK, inFuncToRun }) => {
    if (inUserPK > 0) {
        let LocalReturnData;
        ...
        if (LocalFromFileCheck.KTF) {
            ...
            if (inItemName in LocalReturnData) {
                LocalReturnObject = PrepareScreenObject({
                    inData: LocalReturnData[inItemName],
                    inFuncToRun
                });
            };
        };
    ...
    Object.entries(inData).forEach(
        ([key, value]) => {
            ...
            if ("TableColumns" in value) {
                LocalLoopObject.TableColumnsObject = inFuncToRun({

...
let Update = async ({ DataPK, FolderName, FileName, ItemName, ScreenName, BodyAsJson }) => {
    ...
    const LocalDataToUpdate = (({ ColumnReOrder,ColumnValue,DataAttributesFromTableInfo,DataAttributesFromTableDataRow }) => ({  ColumnReOrder,ColumnValue,DataAttributesFromTableInfo,DataAttributesFromTableDataRow }))(BodyAsJson);
    ...
    if (LocalItemName in LocalNewData) {
        if (LocalScreenName in LocalNewData[LocalItemName]) {
            if ("TableInfo" in LocalNewData[LocalItemName][LocalScreenName]) {
                ...
                LocalNewData[LocalItemName][LocalScreenName].TableInfo.ColumnValue = LocalDataToUpdate.ColumnValue;

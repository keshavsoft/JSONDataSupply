...
let CommonOrderByFuncs = require("./OrderByFuncs/Order");
let CommonArrange = require("./PickColumns/Arrange");
let CommonGroupBy = require("./GroupByFuncs/GroupBy");
let CommonPostGroupBy = require("./GroupByFuncs/PostGroupBy");
//let CommonFromFolder = require("../../Fs/Folders/PullData");
//let CommonFromFolder = require("../../Fs/Config/Folders/PullData/FromAllFiles");
let CommonFromFolder = require("../../Fs/Config/Folders/PullData/getDirectories");
let CommonGroupByFuncsBeforeLast = require("./GroupByFuncs/BeforeLast");
let CommmonPullDataFromVouchersConsiderOnly = require("./PullDataFromVouchersConsiderOnly");
...
let LocalLedgerHelperFuncs = {
            ...
            SubFuncs: {
                LoopReturnDataFuncWithItemName: ({ inReturnData, inItemName }) => {
                    ...
                    if (inItemName in inReturnData) {
                        ...
                        //LocalReturnObject.KResult = Object.values(inReturnData[inItemName]);
    ...
    FilterFuncs: {
        ...
        LoopFunc: ({ inLoopObject, inFilterObject }) => {
            let LocalRetTF = true;
            // console.log("inLoopObject : ", inLoopObject);
        ...
        LoopFunc1: ({ inLoopObject, inFilterObject }) => {
            let LocalRetTF = true;
            if (inLoopObject === undefined) {
...
let FromVouchersConsiderOnlyWithFilters = ({ inLedgerAutoJsonWithItemName, inUserPK, inFiltersData }) => {
    let LocalReturnArray = [];
    ...
    LocalReturnArray = DataFromVouchersConsider({ inVouchersConsider: LocalVouchersConsider, inUserPK });

    LocalFilteredData = LocalLedgerHelperFuncs.FilterFuncs.StartFunc({
        inDataArray: LocalReturnArray,
        inFilterObject: inFiltersData
    });

    if (LocalGroupByKTF) {
        LocalFilteredData = CommonGroupBy.SingleColumnAndMultipleDataRetruned({
            inDataToSort: LocalFilteredData,
            inGroupByColumn: LocalGroupByColumn,
            inColumnsToGroupByAsFloat: LocalGroupByAsFloat
        });

        LocalFilteredData = CommonPostGroupBy.PrepareData({ inData: LocalFilteredData, inDisplayColumns: LocalDisplayColumns });
    };
...
let FromItemNameWithFilters = ({ inLedgerAutoJsonWithItemName, inUserPK, inFiltersData }) => {
    //inFiltersData = {};
    let LocalReturnArray = FromVouchersConsiderOnlyWithFilters({
        inLedgerAutoJsonWithItemName,
        inUserPK, inFiltersData
    });
    let LocalFilteredData;

    CommonArrange.FromTableColumns({ inData: LocalReturnArray, inTableColumns: inLedgerAutoJsonWithItemName.TableColumns });

    return CommonOrderByFuncs.SortData({ inDataToSort: LocalReturnArray, inOrderByColumnsArray: inLedgerAutoJsonWithItemName.OrderByColumns });
};

let LocalBeforeLastFunc = ({ inLedgerAutoJsonWithItemName, LocalReturnArray }) => {
    let LocalSortArray;
    let LocalOrderByColumnsArray;

    let LocalFromBeforeLast = CommonGroupByFuncsBeforeLast.StartFunc({
        inLedgerAutoJsonWithItemName,
        inDataToSort: LocalReturnArray,
        inDisplayColumns: inLedgerAutoJsonWithItemName.DisplayColumns
    });

    if ("OrderBy" in inLedgerAutoJsonWithItemName.BeforeLast) {
        if ("ColumnsArray" in inLedgerAutoJsonWithItemName.BeforeLast.OrderBy && "SortArray" in inLedgerAutoJsonWithItemName.BeforeLast.OrderBy) {
            LocalSortArray = inLedgerAutoJsonWithItemName.BeforeLast.OrderBy.SortArray;
            LocalOrderByColumnsArray = inLedgerAutoJsonWithItemName.BeforeLast.OrderBy.ColumnsArray;

            return CommonOrderByFuncs.SortWithSign({
                inDataToSort: LocalFromBeforeLast,
                inOrderByColumnsArray: LocalOrderByColumnsArray,
                inSortArray: LocalSortArray
            });
        }
    };
};

let FromItemNameWithOutFilters = async ({ inLedgerAutoJsonWithItemName, inUserPK }) => {
    let LocalReturnArray = await CommmonPullDataFromVouchersConsiderOnly.FromVouchersConsiderOnly({ inLedgerAutoJsonWithItemName, inUserPK });
    let LocalReturnData;
...
let FromItemNameWithOutFiltersShowInTableColumnsOnly = async ({ inLedgerAutoJsonWithItemName, inUserPK }) => {
    let LocalReturnArray = await CommmonPullDataFromVouchersConsiderOnly.FromVouchersConsiderOnly({ inLedgerAutoJsonWithItemName, inUserPK });
    let LocalReturnData;
    let LocalReturnObject = { KTF: false, KReason: "" };

    //    let LocalFromArrange = CommonArrange.FromTableColumnsWithTableShowOnly({ inData: LocalReturnArray, inTableColumns: inLedgerAutoJsonWithItemName.TableColumns });

    // console.log("LocalFromArrange : ", LocalFromArrange);
